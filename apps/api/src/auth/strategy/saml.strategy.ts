...
export class SamlStrategy extends PassportStrategy(MultiSamlStrategy, 'saml') {
    constructor(
        ...
        super({
            ...
            getSamlOptions: async (req: e.Request, callback: StrategyOptionsCallback) => {
    ...
    async validate(profile, done) {
        ...
        const user = await this.prismaService.user.findUnique({
            where: { email: typeof profile.email === 'string' ? profile.email : profile.email[0] },
        });
        ...
        if (!providerAccount) {
            await this.authService.createProviderAccount(user, profile, profile.email, profile.email);
        }
